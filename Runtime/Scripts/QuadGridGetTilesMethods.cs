...
{ 
    public static partial class QuadGrid
    {
        ...
        {            
            ...
            public static void TraversalLine(float2 startPosition, float2 targetPosition, int2 mapSize, bool includeOrigin, bool continueTilMapBounds, NativeList<int2> result)
            {
                result.Clear();
                var startCoord = new int2((int)startPosition.x, (int)startPosition.y);
                var targetCoord = new int2((int)targetPosition.x, (int)targetPosition.y);
                if (includeOrigin) result.Add(startCoord);
